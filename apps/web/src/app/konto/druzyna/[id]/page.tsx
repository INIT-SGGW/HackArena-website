                        ...
                        <ol className="w-full max-w-[700px] mx-auto">
                            {
                                mockData.members.map((member, index) => (
                                    <li key={index} className="flex flex-col gap-2 mb-5 last:mb-0">
                                        <div className="flex gap-5 justify-between">
                                            ...
                                            <div className="flex gap-3">
                                                {member.captain && <span className="text-blue-500">Kapitan</span>}
                                                {member.verified ? (
                                                    <span className="text-green-500">Zweryfikowany</span>
                                                ) : (
                                                    <span className="text-error">Niezweryfikowany</span>
                                                )}
                                            </div>
                                        </div>
                                    </li>
                    ...
                    <div className="sm:p-4">
                        {
                            mockData.events.length > 0 ?
                                (
                                    <div className={`flex flex-col md:flex-row gap-5 md:gap-10 ${mockData.events.length > 1 ? "!flex-col !gap-10" : ""}`}>
                                        {mockData.events.length > 0 && (
                                            <NoDescriptionEvents events={mockData.events} compact />
                                        )}
                                    </div>
                                )
                                :
                                (
                                    <p className="text-center">Nie brałeś jeszcze udziału w żadnych wydarzeniach.</p>
                                )
                        }
                    </div>
                </div>
