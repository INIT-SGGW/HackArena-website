...
import { ChangeNotificationStatusResponse, GetNotificationsResponse } from "../../types/responses";
...
export function TeamInvite({ notification, mutate }: Props) {
    ...
    const handleJoin = async () => {
        try {
            setLoading(true);
            setError(null);
            await fetcherHack<null, null>(`/teams/${notification.args.teamId}/members/${userId}/accept-invite`, {
                method: "PATCH",
                headers: {
            ...
            await fetcherAuth<ChangeNotificationStatusRequest, ChangeNotificationStatusResponse>(
    ...
    const handleDecline = async () => {
        try {
            setLoading(true);
            setError(null);
            await fetcherHack<null, null>(`/teams/${notification.args.teamId}/members/${userId}/decline-invite`, {
                method: "PATCH",
                headers: {
                    "Content-Type": "application/json",
                },
        ...
        <li className="flex flex-col gap-3 last:mb-0">
            ...
            <div className="flex flex-col w-full gap-1">
                ...
                <div className="flex  w-full gap-2">
                    <Button fullWidth disabled={loading} onClick={() => handleJoin()}>Dołącz</Button>
                    <Button secondary disabled={loading} fullWidth onClick={() => handleDecline()}>Odrzuć</Button>
                </div>
            </div>
        </li>
    )
