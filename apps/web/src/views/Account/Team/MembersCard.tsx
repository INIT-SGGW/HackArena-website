...
export function MembersCard({ data, isLoading, error, mutate }: Props) {
    ...
    const handleChangeLeader = async (userLeaderId: string) => {
        try {
            await fetcherHack<null, null>(
                `/teams/${teamId}/members/${userLeaderId}/change-leader`,
                {
                    method: 'PATCH',
                },
            );
    ...
    const handleDeleteMember = async (userDeleteId: string) => {
        try {
            await fetcherHack<null, null>(
                `/teams/${teamId}/members/${userDeleteId}/remove-user`,
                {
                    method: 'PATCH',
                },
            );
    ...
    return (
        <div className="flex flex-col gap-10">
            {isLoading && !error && <p className="text-center">Ładowanie...</p>}
            {error && (
                            ...
                            <div className="flex flex-col gap-1">
                                {member.isVerified &&
                                    member.status === MemberStatus.ACCEPTED && (
                                        <span
                                            className={`text-lg font-semibold ${member.isLeader ? 'text-blue-500' : ''}`}
                                        >
            ...
            {captain?._id === userId && data && data.length < 3 && (
                <LinkButton href={window.location.href + '/dodaj'} fullWidth>
                    Dodaj członka
                </LinkButton>
            )}
        </div>
...
const OptionsButtons = ({
    ...
    return (
        <button
            {...props}
            onClick={onClick}
            className={`flex flex-1 sm:flex-auto items-center justify-center w-12 sm:w-10 h-10 rounded-md cursor-pointer ${props.className}`}
            title={title}
        >
            <Image
                title={title}
