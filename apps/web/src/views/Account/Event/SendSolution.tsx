...
export function SendSolution() {
    ...
    const { data, error: fileInfoError, isLoading, mutate } = useSWR<
        GetTeamFileInfoResponse,
        Error
    >(`/teams/${id}/bot/info`, (url: string) =>
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            await fetcherHack<FormData, null>(
                `/teams/${id}/bot`,
                {
                    method: 'POST',
                    body: formData,
    ...
    return (
        <>
            <div>
                ...
                {
                    fileInfoError && <p className="text-primary text-xl font-bold">Nie przesłano jeszcze żadnego rozwiązania</p>
                }
                {
                    !fileInfoError && !data && isLoading && <p className="text-primary text-xl font-bold">Ładowanie...</p>
                }
                {
                    !fileInfoError && data && (
                        <p className="text-primary text-xl font-bold">
                            {toDateString(new Date(data.dateCreated))} - {(data.fileSize / 1024 / 1024).toFixed(2)} MB
                        </p>
                    )
                }
            </div>
                ...
                <div className="flex flex-col gap-1">
                    ...
                    >
                        ...
                        >
                            ...
                            <span className="text-background text-bold text-xl">{message}</span>
                        </div>
                    </label>
