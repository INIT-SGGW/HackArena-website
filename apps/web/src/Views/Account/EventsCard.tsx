            ...
            <div className="sm:p-4">
                {
                    [...mockEvents.current, ...mockEvents.past].length > 0 ?
                        (
                            <div className={`flex flex-col md:flex-row gap-5 md:gap-10 ${mockEvents.current.length > 1 ? "!flex-col !gap-10" : ""}`}>
                                {mockEvents.current.length > 0 && (
                                    <>
                                        <NoDescriptionEvents title="Nadchodzące" events={mockEvents.current} compact />
                                        <div className={`w-[2px] bg-secondary-200 my-[70px] hidden md:block ${mockEvents.current.length > 1 ? "!hidden" : ""}`} />
                                    </>
                                )}
                                {mockEvents.past.length > 0 && <NoDescriptionEvents grow title="Zakończone" events={mockEvents.past} compact />}
                            </div>
                        )
                        :
                        (
