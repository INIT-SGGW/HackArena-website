...
export function SolutionsTable() {
    const { data, error: fileInfoError, isLoading } = useSWR<
        GetTeamFileInfoResponse[],
        Error
    >(`/teams/bots`, (url: string) =>
    ...
    const downloadFile = async (teamName: string, teamId: string) => {
        const blob = await fetcherHack<null, Blob>(`/teams/${teamId}/bot/file`, {
            method: 'GET',
            responseType: 'blob',
        });
    ...
    return (
        <div className="flex flex-col gap-5 w-full items-center">
            ...
            {
                fileInfoError && <p className="text-xl font-bold">Nie przesłano jeszcze żadnego rozwiązania</p>
            }
            {
                    ...
                    <table className="w-full">
                        ...
                        <tbody>
                            {data.map((file) => (
                                <tr key={file.teamId}>
                                    <td>{file.teamName}</td>
                                    <td>{toDateString(new Date(file.dateCreated))}</td>
                                    <td>{(file.fileSize / 1024 / 1024).toFixed(2)} MB</td>
                                    <td>
                                        <Button
                                            type="button"
                                            className="py-0"
                                            onClick={() => downloadFile(file.teamName, file.teamId)}
                                        >
                                            Pobierz
                                        </Button>
                                    </td>
